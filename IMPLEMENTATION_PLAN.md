...
**Total backend test files**: 47 spec files
...
| Layout (sidebar + topbar) | Needs rewrite | 1 test file | Desktop + mobile sidebars synced: 2 nav items (Dashboard + Notifications) + 2 general (Settings, Help) |
| Shared (API client, error boundary, button) | Complete | 2 test files | CSRF, locale, error handling |

**Total frontend test files**: 11 test files
...
- [ ] **Verify Prisma schema ARCHIVED comments** — Confirm all 18 cap-table models have `// ARCHIVED` comments. Verified: no active module imports these models (only ReportsService queries them via Prisma client, which is acceptable since the models remain in the schema). Note: the `NOTIFICATION_TYPE_CATEGORY` in `create-notification.dto.ts` still maps old cap-table notification types to categories — these are string constants, not model imports. The `Prisma.ShareholdingWhereInput` type reference in `reports.service.ts` line 41 depends on the archived model staying in the schema. | `backend/prisma/schema.prisma` | **S**
- [x] **Run full backend test suite and fix failures** — 47/47 test suites pass, 1,205/1,205 tests pass. All active module tests green. No test files for deleted modules remain (they were already removed from working tree). | All backend spec files | **M**
...
- [ ] **Clean up i18n messages** — Remove cap-table namespaces from `messages/en.json` and `messages/pt-BR.json` (capTable, shareholders, shareClasses, transactions, fundingRounds, optionPlans, convertibles, documents, auditLogs). Keep dashboard, common, errors, settings, notifications. Reports namespace will be rewritten. | `frontend/messages/en.json`, `frontend/messages/pt-BR.json` | **M**
...
- [x] **Run full frontend test suite and fix failures** — 11/11 test suites pass, 255/255 tests pass. All active page tests green. No test files for deleted pages remain (they were already removed from working tree). | All frontend test files | **M**
...
- [ ] **Update specs/README.md** — Change "Planned Specs" section to "Active Specs" since all 5 "planned" specs (ai-document-intelligence.md, investor-qa.md, open-finance.md, investor-portal.md, frontend-restructure.md) already exist. Update the count from 11 to 16. Add company-data-enrichment.md to the list. | `specs/README.md` | **S**
...
- [ ] **Auth refresh endpoint** — `POST /api/v1/auth/refresh` to extend sessions without full re-login. Privy token refresh + session extension. Per `authentication.md` spec. | `backend/src/auth/auth.controller.ts`, `auth.service.ts` | **M**
- [ ] **KYC gating guard** — Middleware/guard that blocks certain actions (e.g., publishing a profile) unless the founder has KYC APPROVED status. Per `kyc-verification.md`: founders must be verified before publishing. Implementation: create `KycGatingGuard` that reads user's `kycStatus` from DB and throws 403 if not APPROVED. Apply via `@UseGuards(KycGatingGuard)` on profile publish endpoint. | `backend/src/kyc/guards/kyc-gating.guard.ts` | **M**
...
- [ ] **New sidebar navigation** — Replace current 2-item sidebar with founder dashboard nav per `frontend-restructure.md` § "Sidebar Navigation": Dashboard, Company Page (profile editor), Dataroom, Reports (AI reports, stub until Phase 2), Q&A (conversations, stub until Phase 2), Updates (founder posts, stub until Phase 4), Bank Connections (stub until Phase 3), Analytics (profile view analytics). Plus existing: Notifications, Settings, Help. Stubs show "Coming Soon — Disponível em breve" card with i18n. | `frontend/src/components/layout/sidebar.tsx`, `mobile-sidebar.tsx` | **M**
- [ ] **Dashboard page redesign** — Replace 3 generic stat cards with meaningful dashboard per `frontend-restructure.md` § 4: Profile completeness progress bar (% of profile fields filled + team added + documents uploaded + KYC verified), Recent activity feed (last 10 audit log events for the company), Quick actions (Edit Profile, Upload Document, Share Page, View Public Page), Company health summary card (KYC status, CNPJ validation status, enrichment status, litigation risk). | `frontend/src/app/(dashboard)/dashboard/page.tsx` | **L**
- [ ] **Company Page builder** — Profile editor with tabs per `frontend-restructure.md` § 5: **Overview tab** (company name, description rich text, sector dropdown, headline, logo upload, banner upload), **Metrics tab** (up to 6 custom metrics: label + value + optional trend, drag to reorder), **Team tab** (up to 10 members: name, role, bio, photo upload, LinkedIn URL), **Share tab** (slug editor with preview URL, access type selector: PUBLIC/PASSWORD/EMAIL_GATED, password field for PASSWORD type), **Preview tab** (live preview of how the public page looks). Backend endpoints already exist in CompanyProfileController (11 endpoints). Frontend needs: new route group, tab layout, form components, S3 upload integration for images, slug validation, auto-save. | `frontend/src/app/(dashboard)/dashboard/company-page/` (new) | **XL**
- [ ] **Dataroom page** — Document management per `frontend-restructure.md` § 6 and `company-dataroom.md` spec: Category-based organization (PITCH_DECK, FINANCIALS, LEGAL, PRODUCT, TEAM, OTHER), Drag-and-drop file upload with progress indicator, Document list with columns (name, category, size, uploaded date, download count), Download/delete action buttons per document, Storage usage indicator (25MB/file, 500MB total), Reorder documents within category. Backend endpoints already exist in ProfileDocumentController (5 endpoints). Frontend needs: new page with upload dropzone, category filter tabs, file list table, storage bar. | `frontend/src/app/(dashboard)/dashboard/dataroom/` (new) | **L**
...
- [ ] **i18n keys for all new pages** — Add all translation keys for Company Page builder, Dataroom, Public Profile, Dashboard redesign, KYC flow, Settings updates, enrichment section, access gate to both `pt-BR.json` and `en.json`. Estimate ~200 new keys per locale. Follow existing naming convention: `{feature}.{element}.{descriptor}`. | `frontend/messages/en.json`, `frontend/messages/pt-BR.json` | **M**
...
- [ ] **Frontend test suite green** — After Phase 0 cleanup, run full test suite and verify 100% pass rate. Write tests for new pages: Company Page builder, Dataroom, Public Profile, KYC flow, Settings updates. Target: >80% statement coverage. Follow `testing-frontend.md` rules (mock APIs, test rendering/interactions/accessibility). | All frontend test files | **L**
