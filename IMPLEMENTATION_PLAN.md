...
**Frontend**: P4.1 Foundation complete (Privy auth, next-intl v4, shadcn/ui, typed API client with CSRF/i18n/401 handling). P4.3 Dashboard Page complete. P4.4 Cap Table Page complete (3-tab view, data tables, export, 5 TanStack Query hooks). P4.5 Shareholders Page complete (list table, search/filters, CPF masking, delete dialog). P4.6 Share Classes Page (list) complete (4 stat cards, type filter, data table with 8 columns, delete confirmation, pagination, 22 tests). P4.7 Transactions Page (list) complete (4 stat cards, type+status filters, 9-column data table with type/status badges, cancel dialog, pagination, BRL currency formatting, 25 tests). P4.8 Funding Rounds Page (list) complete (4 stat cards Total/Open/Closed/Draft with ocean-600 highlight, type+status filter dropdowns, 8-column data table with type/status badges, cancel dialog for DRAFT/OPEN rounds, BRL currency formatting, pt-BR date formatting, client-side type filtering, 24 tests). P4.10 Convertible Instruments Page (list) complete (4 stat cards Total/Outstanding/Principal/Accrued Interest with ocean-600 highlight, type+status filter dropdowns, 9-column data table with type/status badges, cancel dialog for cancellable statuses, BRL currency formatting, pt-BR date formatting, pagination, 25 tests, full i18n convertibles.* namespace PT-BR+EN). CompanyProvider context for selected company state with localStorage persistence. TypeScript types for API responses (`types/company.ts`). 234 tests across 11 test suites.
