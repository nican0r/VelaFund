...
- [x] **Public profile page** — `/p/[slug]` route with state machine architecture (loading → profile/email_gate/password_gate/not_found/error). Minimal public layout (no sidebar/auth). Profile content: Header (logo or initial fallback, company name, headline, sector badge with 26 sector labels, founded year, location, website link), About section (description), Key Metrics grid (cards with Brazilian formatting — CURRENCY_BRL via `Intl.NumberFormat('pt-BR')`, CURRENCY_USD, PERCENTAGE, NUMBER, TEXT), Team section (member cards with photo or initials avatar, name, title, LinkedIn link), Documents section (file list with category badge, size, page count, download via pre-signed URL from PublicDocumentController), Litigation section (3 states: PENDING spinner, FAILED warning, COMPLETED with risk level badge and lawsuit count/value). Navia-branded header and footer. Added `PublicProfile` + related types to `types/company.ts`. Added `publicProfile.*` i18n namespace (~30 keys per locale). 26 tests covering all states, gates, formatting, and API calls. 16/16 frontend suites (426 tests), build passes. | `frontend/src/app/p/[slug]/page.tsx`, `frontend/src/app/p/layout.tsx`, `frontend/src/types/company.ts` | **XL**
- [x] **Public profile access gate** — Integrated into public profile page as part of the state machine. PASSWORD gate: password input form, validates via `GET /api/v1/profiles/:slug?password=`, shows "Incorrect password" inline error on 401, transitions to profile on success. EMAIL_GATED gate: email input form, submits via `GET /api/v1/profiles/:slug?email=`, transitions to profile on success, shows error on failure. Both gates: disabled submit when empty, loading spinner during submission. Profile data completely hidden until gate passed. | `frontend/src/app/p/[slug]/page.tsx` | **M**
...
- [ ] **i18n keys for remaining new pages** — Company Page builder (~90 keys), Dataroom (~40 keys), and Public Profile (~30 keys) i18n done. Still needed: KYC flow, Settings updates, enrichment section. Estimate ~60 remaining keys per locale. Follow existing naming convention: `{feature}.{element}.{descriptor}`. | `frontend/messages/en.json`, `frontend/messages/pt-BR.json` | **S**
