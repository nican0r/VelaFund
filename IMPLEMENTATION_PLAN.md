...
- [x] Dead letter queue monitoring — DONE (v0.0.76): ScheduledTasksService.monitorDeadLetterQueues() runs every 5 minutes via @Cron('0 */5 * * * *'). Checks getFailedCount() across all 6 Bull queues (audit-log, notification, company-setup, report-export, kyc-aml, profile-litigation). Logs WARNING at ≥10 failed + fires DLQ_WARNING_ALERT audit event. Logs CRITICAL at ≥50 failed + fires DLQ_CRITICAL_ALERT audit event. Skips unreachable queues gracefully. 15 new DLQ tests (36 total scheduled-tasks tests).
- [x] PostgreSQL immutability trigger — DONE (v0.0.76): AuditLogModule.onModuleInit() runs 5 idempotent SQL statements on startup: CREATE OR REPLACE FUNCTION prevent_audit_modification() (plpgsql, RAISE EXCEPTION on UPDATE/DELETE), DROP/CREATE TRIGGER audit_logs_immutable_update (BEFORE UPDATE), DROP/CREATE TRIGGER audit_logs_immutable_delete (BEFORE DELETE). Graceful degradation if DB unavailable. 11 tests.
...
**Tests**: 998 frontend tests (11 api-client + 4 error-boundary + 10 button + 27 company-switcher + 22 onboarding + 2 remaining + 922 page/component tests)
...
- [x] Document TypeScript types (DocumentStatus, DocumentTemplateType, DocumentTemplate, DocumentItem) in `types/company.ts` — DONE
- [x] TanStack Query hooks (`use-documents.ts`): 11 hooks (useDocumentTemplates, useDocumentTemplate, useDocuments, useDocument, useDocumentPreview, useDocumentDownloadUrl, useCreateDocument, useCreateDraft, useUpdateDraft, useGenerateFromDraft, useDeleteDocument) — DONE
- [x] Documents i18n namespace (~100 keys PT-BR+EN): list, wizard, detail, upload, delete, pagination, status/type labels — DONE
- [x] Documents list page: 4 stat cards (Total/Drafts/Generated/Pending) with ocean-600 highlight, search input + type/status filter dropdowns, data table (title link/template type/status badge/date/action buttons), pagination, delete confirmation dialog, upload dialog (PDF/JPEG/PNG validation, 10MB limit, FormData upload), loading skeleton, empty/error/no-company states — DONE
- [x] Document creation wizard: 2-step flow (template selection grid with 5 type icons → dynamic form + live preview split pane), DynamicFormField renderer (text/number/date/boolean/select/textarea), Save Draft + Generate PDF actions with mutations, template formSchema parsing — DONE
- [x] Document detail page: header with title + status badge + template subtitle, conditional actions (Edit Draft link/Generate PDF/Download/Delete), two-column layout (preview HTML via dangerouslySetInnerHTML or empty state / metadata card + form data card), content hash truncation, locale display, delete confirmation dialog with redirect — DONE
...
- [x] TanStack Query hooks (`use-audit-logs.ts`): 3 hooks (useAuditLogs with 8 filter params, useAuditLog detail, useVerifyHashChain with optional enabled flag) — DONE
- [x] Audit logs i18n namespace (~120 keys PT-BR+EN): title, description, stats, table, filters, 46 action labels, 16 resource types, 3 actor types, changes, metadata, verify, empty, pagination, error — DONE
- [x] Audit logs list page: 4 stat cards (Total Events active/highlighted, Today's Events, User Actions, System Events), action dropdown filter (~46 options), resource type dropdown filter (~16 options), date range filters (from/to), clear filters button, data table with expandable rows (timestamp, actor with icon, action badge with color coding, resource type, truncated resource ID), pagination — DONE
- [x] Expandable rows: click to expand showing before/after JSON changes (color-coded labels red/green) and metadata (IP address, request ID, source, user agent) — DONE
- [x] Hash chain verification section: Verify button triggers useVerifyHashChain, shows VALID/INVALID/NO_DATA status with icons (ShieldCheck/ShieldAlert/FileSearch), days verified/valid/invalid stats, loading spinner — DONE
- [x] Action badge color coding: green for created/issued/approved/success, red for failed/rejected, gray for deleted/cancelled, blue for updated/changed, cream for submitted/opened/started — DONE
- [x] Actor type display: Bot icon for SYSTEM, Shield for ADMIN, Users for USER; shows actor name or type label, masked email underneath — DONE
- [x] Loading/error/empty/no-company states — DONE
...
- [x] PostgreSQL immutability trigger for audit_logs — DONE (v0.0.76): via onModuleInit, not migration
...
- [x] `docs/user-flows/audit-logging.md` — DONE (v0.0.76, updated v0.0.78 with frontend flows)
